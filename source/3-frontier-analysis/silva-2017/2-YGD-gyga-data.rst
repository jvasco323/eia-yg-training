...
This notebook complements an earlier notebook describing the methodology
...
An example on how data can be accessed from GYGA, and linked to farmer
...
The next step entails merging the yield data from GYGA with the
...
using the *st_as_sf()* function. Please refer to the chunk of code
below.
...
using the *st_join()* function. This is described in the chunk of code
below.
...
The next step is to get the closest weather station to each field within
...
Now, the fields with water-limited yield data from the same climate zone
...
source of the water-limited yield data.
...
Some fields do not fall inside climate zones for which crop model
...
the closest weather station is estimated using the chunk of code below.
...
Fields with a weather station closest than 30km are identified first.
...
Lastly, crop yield data need to be retrieved for the fields located more
...
The last step in this workflow is to bring back the different subsets of
...
Please note this script does not add data to fields without GPS
coordinates reported. The national average could also be considered for
those fields, but this is not implemented in this script.

Data from GYGA can be made available to the OneCGIAR initiative on
**Excellence in Agronomy** on a demand basis. Requests should be made to
prof. Martin van Ittersum (**martin.vanittersum@wur.nl**). The script
...
**j.silva@cgiar.org** or **marloes.vanloon@wur.nl**.
